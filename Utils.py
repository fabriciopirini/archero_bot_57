    ...
    with open(path, "w") as json_file:
        ...
        for coord, value in data.items():
            ...
            for attr, val_Attr in value.items():
                attrs.append(
                    '{}"{}": {}'.format(spaces + spaces, attr, json.dumps(val_Attr))
                )
            formatted_attrs = ",\n".join(attrs)
            main_attrs.append(
                '{}"{}":{}{}'.format(
...
def readAllSizesFolders() -> dict:
    folders = [
        f for f in os.listdir("datas") if os.path.isdir(os.path.join("datas", f))
    ]
    dataFolders = {}
...
def getCoordFilePath(
    dict_name: str, size: tuple = (None, None), sizePath: str = ""
) -> str:
    """
    ...
    if sizePath == "" and size == (None, None):
        raise Exception(
            "Unable to get path for coordinate %s. No correct size or path given"
            % dict_name
        )
